...
Flint.prototype.debug = function(message) {
  ...
  message = moment().utc().toDate() + " " + message;
...
Flint.prototype.onMessageCreated = function(message) {
  ...
  // if bot not found...
  if (!bot) {
    if (this.retryMessageCreated >= 10) {
      return when(False)
    } else {
      this.retryMessageCreated += 1;
      let flintObj = this;
      return delay(1000).then(() => {
        flintObj.debug(`retry processing message #${flintObj.retryMessageCreated} from ${message.personEmail}`);
        flintObj.onMessageCreated(message);
      });
    }
  }
  else {
